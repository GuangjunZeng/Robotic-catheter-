...
parser.add_argument('--Obs_V', type=int, default= 9, help='maximal obstacle velocity, cm/s')
...
parser.add_argument('--basic_ctrl_interval', type=float, default=0.25, help='control interval (s), 0.1 means 10 Hz control frequency')
parser.add_argument('--ctrl_delay', type=int, default=0, help='control delay, in basic_ctrl_interval, 0 means no control delay')
parser.add_argument('--K', type=tuple, default=(0.05,0.5), help='K_linear, K_angular')
...
agent.queue.clear()
s, info = envs.reset()
initial_TW_s = agent.queue.get() #(N, T, D) --> torch.Size([1, 10, 32])
#print("initial_TW_s: ", initial_TW_s.size())
#print(len(agent.queue))
r = 0
dw = False
tr = False
...
def action_to_expected_car_position(a, temp_car_state, arange_constant, ctrl_interval, K, a_space):
    ...
    #temp_car_state[3:5] = K*temp_car_state[3:5] + (1-K)*temp 已经转移到函数
    
    # 速度向量 [v_l, v_l] 
    velocity = np.array([temp_car_state[3], temp_car_state[3]])
    print("In action_to_expected_car_position(), velocity: ", velocity)
    
    # 方向向量 [cos(theta), -sin(theta)] 
    #direction = np.array([np.cos(temp_car_state[2]), -np.sin(temp_car_state[2])]) 
    direction = np.array([np.cos(temp_car_state[2]), -np.sin(temp_car_state[2])]) 

    ls = ctrl_interval * velocity * direction
    euclidean_distance = np.sqrt(ls[0]**2 + ls[1]**2)
    print("delta_move_distance: ", euclidean_distance)
...
while True:
    ...
    try:
        ...
        #! 太多重复点的信息加进去了? 可以sample一下?
        if envs.step_counter_vec > 110:
        ...
        #print("In while loop, a_numpy: ", a_numpy)
        ...
        if envs.step_counter_vec < time_begin_action: #当没到时间时，next_relative_desired_point根本不会有任何作用影响
            temp_car_state_numpy = envs.car_state.cpu().numpy()[0]
            next_car_position = action_to_expected_car_position(a_numpy, temp_car_state_numpy, arange_constant_numpy, ctrl_interval_numpy, K_numpy, a_space_numpy)
            next_relative_desired_point = next_car_position - translation
            stored_next_relative_desired_point = next_car_position - translation
        else:
            if envs.step_counter_vec == time_begin_action:
                #在开始运动时，把运动初始化为直接指向target point. 再没有真正执行下一个action之前，运动方向一直不变
                new_delta = envs.target_point - envs.car_state[:, 0:2]
                new_orientation = torch.atan2(new_delta[:, 1], new_delta[:, 0]) #形状为(N,)的一维PyTorch 张量
                #!进行转换成互补角，符合定义的坐标系
                new_orientation = (2 * torch.pi - new_orientation) % (2 * torch.pi)  # 互反角，范围 [0, 2π]
                print("At the beginning of moving, new_orientation: ", new_orientation)
                envs.car_state[:, 2] = new_orientation
                #envs.car_state[:, ]

            judge_distance = np.linalg.norm(stored_next_relative_desired_point - [relative_real_x_color, relative_real_y_color])
            #envs.car_refresh_counter += 1
            #if envs.car_refresh_counter > envs.car_refresh_interval:
            if judge_distance < 12:
                #复刻仿真中的限制变化幅度, v_l和v_a的变化(即:action)
                envs.car_state[:,3:5] = envs.K * envs.car_state[:,3:5] + (1-envs.K)*envs.a_space[envs.arange_constant,a]
                temp_car_state_numpy = envs.car_state.cpu().numpy()[0]
                next_car_position = action_to_expected_car_position(a_numpy, temp_car_state_numpy, arange_constant_numpy, ctrl_interval_numpy, K_numpy, a_space_numpy)
                next_relative_desired_point = next_car_position - translation

                #用理论计算更新orientation
                envs.car_state[:,2] += envs.ctrl_interval.squeeze(-1) * envs.car_state[:,4]
                #!先用理论值计算orientation试试， oritention本身的值对不对呢? 
                #更新错误，orientation的更新是用用于下一轮运动方向的
                '''delta_y = next_relative_desired_point[1] - stored_next_relative_desired_point[1]
                delta_x = next_relative_desired_point[0] - stored_next_relative_desired_point[0]
                new_orientation = math.atan2(delta_y, delta_x) 
                #!进行转换成互补角，符合定义的坐标系
                new_orientation = (2*math.pi - new_orientation) % (2 * math.pi)  # 转换到 [0, 2π]  # 结果在 [0, 2π]  
                envs.car_state[:, 2] = torch.full(
                        (envs.car_state.shape[0],),  # 显式构造 (N,) 形状
                        new_orientation,
                        device=envs.dvc,
                        dtype=torch.float32
                    )'''
