...
def color_point_detect(color_array1, color_array2, left1, top1, left2, top2, delta1, delta2):
    ...
    # 设置红色阈值（HSV颜色空间）
    lower_red1 = np.array([0, 40, 120])   #[0, 3, 3]      #降低饱和度和明度的下限
    upper_red1 = np.array([18, 120, 195]) #[18, 255, 255] #稍微扩大色调范围
    lower_red2 = np.array([145, 50, 130])
    upper_red2 = np.array([182, 120, 195])
    ...
    if num_contours1 > num_contours2:
        ...
        current_hsv = hsv1  # 新增：记录当前处理的HSV图像
        current_bgr = bgr_image1  # 新增：记录当前处理的BGR图像
    else:
        current_hsv = hsv2  # 新增：记录当前处理的HSV图像
        current_bgr = bgr_image2  # 新增：记录当前处理的BGR图像
    ...
    for cnt in contours:
        ...
        if circularity > 0.035:  # 圆度阈值（1为完美圆）
            # 计算中心坐标
            M = cv2.moments(cnt)
            if M["m00"] != 0:
                cx = int(M["m10"] / M["m00"])
                cy = int(M["m01"] / M["m00"])
                ########### 新增：打印像素信息 ###########
                # 获取HSV和BGR像素值（注意坐标顺序为[y, x]）
                hsv_values = current_hsv[cy, cx]
                bgr_values = current_bgr[cy, cx]
                # 转换BGR到RGB
                rgb_values = (bgr_values[2], bgr_values[1], bgr_values[0])
                print(f"HSV: {hsv_values}, RGB: {rgb_values}")
