...
default_cfg = dict(
    ...
    basic_ctrl_interval=0.3, # control interval (s), 0.1 means 10 Hz control frequency

    ctrl_delay=0, # control delay, in basic_ctrl_interval, 0 means no control delay

    K=(0.1,0.1), # K_linear, K_angular   这两个系数用来调节机器人在控制命令执行时的惯性和响应特性。例如，在机器人接收到新的目标速度指令时，会将新的指令与当前状态（或先前的速度）按照 K 的权重进行混合，从而实现平滑过渡
