...
parser.add_argument('--K', type=tuple, default=(0.05,0.55), help='K_linear, K_angular')
...
def action_to_expected_car_position(a, temp_car_state, arange_constant, ctrl_interval, K, a_space):
    ...
    temp_car_state[3:5] = K*temp_car_state[3:5] + (1-K)*temp
    
    # 速度向量 [v_l, v_l] 
    velocity = np.array([temp_car_state[3], temp_car_state[3]])
    print("In action_to_expected_car_position(), velocity: ", velocity)
    #velocity = np.array([temp_car_state[3], temp_car_state[3], temp_car_state[4]])
    
    # 方向向量 [cos(theta), -sin(theta)] 应该没有负号?
