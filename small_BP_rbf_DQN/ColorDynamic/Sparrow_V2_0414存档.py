...
# handlers.py
#from sockets import global_socket_client1
#from sockets import get_client1

# 19 editable configurations:
default_cfg = dict(
    dvc=torch.device('cuda'), # running device of Sparrow: cuda / cpu
    ...
    Obs_R=12,  # maximal obstacle radius, cm
    Obs_V=11,  # maximal obstacle velocity, cm/s
    ...
    K=(0.05,0.55), # K_linear, K_angular   这两个系数用来调节机器人在控制命令执行时的惯性和响应特性。例如，在机器人接收到新的目标速度指令时，会将新的指令与当前状态（或先前的速度）按照 K 的权重进行混合，从而实现平滑过渡
...
class Sparrow():
    def __init__(self, **params):
        ...
        assert self.ld_num % self.ld_GN == 0  # ld_num must be divisible by ld_GN。其作用是检查条件是否为真。如果条件不满足，则会抛出 AssertionError
        ...
        #仅适用于离散动作空间
        self.a_space = torch.tensor([[0.2*self.v_linear_max , self.v_angular_max],  # 动作 0：低速前进 + 右转
                                     [self.v_linear_max , self.v_angular_max], # 动作 1：全速前进 + 右转
                                     [self.v_linear_max, 0], # 动作 2：全速前进
                                     [self.v_linear_max, -self.v_angular_max],     # 动作 3：全速前进 + 左转
                                     [0.2*self.v_linear_max, -self.v_angular_max], # 动作 4：低速前进 + 左转
                                     [-self.v_linear_max, 0],  # 动作 5：全速后退
                                     [0.1*self.v_linear_max, 0], # 动作 6：减速
                                     [0., 0.]], device=self.dvc) # 动作 7：停止

        # 仅适用于离散动作空间
        self.a_space = self.a_space.unsqueeze(dim=0).repeat((self.N, 1, 1)) # (action_dim,2) -> (N,action_dim,2)

        # 仅适用于离散动作空间
        self.a_state = torch.tensor([[0.2,1], [1,1], [1,0], [1,-1], [0.2,-1], [-1,0], [0.1,0], [0,0]], device=self.dvc) # (action_dim,2) 给出了每个离散动作对应的归一化表示

        if self.action_type == 'Discrete': self.init_pre_action = (self.action_dim-1)*torch.ones(self.N, dtype=torch.int64, device=self.dvc)
        else: self.init_pre_action = torch.zeros((self.N,2), device=self.dvc) # the pre_action of init state。 当使用连续动作时，每个环境的初始动作被初始化为 [0, 0]，代表没有任何控制输入

        self.arange_constant = torch.arange(self.N, device=self.dvc) # 仅用于索。这行代码创建了一个从 0 到 N-1 的整数张量，其中 N 表示向量化环境的数量。这个张量通常用于后续在多环境并行时进行索引操作，方便对每个环境的状态或动作进行独立访问。
        self.K = torch.tensor([self.K], device=self.dvc) # K_linear, K_angular。 转换为一个张量。

        self.ctrl_interval = self.basic_ctrl_interval * torch.ones((self.N,1), device=self.dvc) #基本控制间隔（basic_ctrl_interval，例如 0.1 秒）生成一个形状为 (N,1) 的张量

        self.ctrl_pipe_init = deque() #双端队列（deque），用于保存由于控制延迟而需要排队等待执行的控制命令。
        ...
        self.car_refresh_interval = 3
        self.car_refresh_counter = 0
        ...
        self.ld_scan_result = torch.zeros((self.N, self.ld_num), device=self.dvc)  # used to hold lidar scan result, (N, ld_num).  ld_num is the number of lidar streams in each world.
        ...
        if self.noise:
            self.noise_magnitude = torch.hstack((torch.tensor([2,2,torch.pi/50,1,torch.pi/50]), torch.ones(self.grouped_ld_num))).to(self.dvc) #(23,)
        ...
        self.reward_vec = torch.zeros(self.N, device=self.dvc) # 创建了一个形状为 (N,) 的张量，用来存储每个环境在当前步的奖励值（reward signal）
        ...
        self.state_upperbound = torch.ones(self.state_dim-4, device=self.dvc) # -4 表示排除了 act_state 部分
    ...
    def _Domain_Randomization(self): #添加噪声，增加鲁棒性
    ...
    def _dynamic_obstacle_init(self):
        ...
        if self.RdON:
            for i in range(self.N): self.dynamic_obs_mask[i, int(self.O - i * self.O / self.N):, :, :] = False
        ...
        self.vec_dynamic_obs_P += torch.ones((self.N, self.O, 1, 2),dtype=torch.long,device=self.dvc)*(self.R_map-self.Obs_R-self.w)*self.dynamic_obs_mask  # 平移至中心分散
        disperse = self.R_map - 2*(self.Obs_R + self.w)
        self.vec_dynamic_obs_P += torch.randint(-disperse, disperse, (self.N, self.O, 1, 2), device=self.dvc)*self.dynamic_obs_mask # 随机分散
        ...
        # 随机每个env的最大速度
        if self.ScOV:
            random_idx = torch.randperm(self.N) # 对每个env intance随机，防止env=N的障碍物又最少，速度又最慢
            scale = torch.linspace(1, 0.6, self.N)[random_idx]
            self.max_Obs_Vs = (self.max_Obs_V * scale.reshape((self.N,1,1,1))).long().to(self.dvc)
            # print(self.max_Obs_Vs.squeeze())

    def _rect_in_bound(self, N:int, x:int, y:int, range:int) -> bool:
        '''这个函数的作用是判断给定环境中，一个以 (x,y) 为中心、边长为 2×range 的矩形区域内是否包含障碍物。
        它首先通过生成该区域内所有的坐标点，然后对这些点进行编码，并与该环境中所有障碍物的编码进行比较。
        如果有任一坐标点的编码与障碍物编码匹配，则说明该区域内有障碍物。'''
        ...
        return ((rect_code - self.vec_bound_code[N])==0).any() # (X*2,1)-(1,O1*P1+O2*P2+O3*P3)
    ...
    def _target_point_init(self, N:int):
        ...
        while True:
            ...
            if self._rect_in_bound(N, x, y, self.target_area+self.car_radius): continue # 与障碍物重合，重新生成
    ...
    def _car_loc_init(self, N:int):
        ...
        while True:
            ...
            # 设定固定的 x 和 y 坐标（示例值，你可以替换成你想要的数值）
            fixed_x = 200  # x偏移量：500
            fixed_y = 150  # y偏移量：400

            # 创建与原来相同格式的张量（shape=(2,), 数据类型和设备一致）
            loc = torch.tensor([fixed_x, fixed_y], dtype=torch.int64, device=self.dvc)

            if self._rect_in_bound(N, loc[0].item(), loc[1].item(), 4*self.car_radius):
                print("car's initial position coincides with the obstacles~")
    ...
    def reset(self):
        ...
        #步数初始化
        self.step_counter_vec.fill_(0) #使用了 tensor 的 in-place 方法 fill_()，将 self.step_counter_vec 张量中的所有元素都重置为 0。
        ...
        self._reward_function(self.init_pre_action) #这只是reset时计算reward用的
        ...
        if self.done_vec.any():
            self.reset_cnt += 1
            if self.reset_cnt > 100: print(f"Cannot reset env after {self.reset_cnt} tries. May consider reduce Obs_V.")
            return self.reset()
    ...
    def _obstacle_move(self):
        ...
        if self.RdOV: self.Obs_V_tensor += torch.randint(-1,2,(self.N, self.O, 1, 2),device=self.dvc) # 每次速度改变量∈[-1,0,1]

        # 限速
        if self.ScOV: self.Obs_V_tensor.clip_(-self.max_Obs_Vs, self.max_Obs_Vs) # max_Obs_Vs是张量，(N,1,1,1)
        else: self.Obs_V_tensor.clip_(-self.max_Obs_V, self.max_Obs_V) # max_Obs_V是标量

        # 移动障碍物, 注意vec_dynamic_obs_P_shaped会与vec_dynamic_obs_P数据联动
        self.vec_dynamic_obs_P += self.Obs_V_tensor*self.dynamic_obs_mask  # (N,O,P,2) += (N,O,1,2)
    ...
    def _ld_not_in_bound_vec(self):
        ...
        pre_goon = self.ld_end_code[:, :, None] - self.vec_bound_code # (N,ld_num,1)-(N,1,O1*P1+O2*P2)
    ...
    def _ld_scan_vec(self):
        ...
        self.ld_vectors_wd = torch.stack((torch.cos(self.ld_angle), -torch.sin(self.ld_angle)), dim=2)
        self.ld_end_wd = self.car_state[:,None,0:2] + self.car_radius * self.ld_vectors_wd  # 扫描过程中，雷达射线末端世界坐标(初始化于小车轮廓), (N,1,2)+(N,ld_num,2)=(N,ld_num,2)。 从小车中心出发，沿每条射线的方向延伸 car_radius 的距离，得到射线的起点（位于小车边缘）。
        self.ld_end_gd = self._world_2_grid(self.ld_end_wd)  # 扫描过程中，雷达射线末端栅格坐标, (N,ld_num,2)。 转换射线末端到栅格坐标
        self.ld_end_code = self.ld_end_gd[:,:,0]*self.window_size + self.ld_end_gd[:,:,1]# 扫描过程中，雷达射线末端栅格坐标的编码值, (N,ld_num)。 快速判断射线末端是否碰到障碍物（通过检查编码是否在障碍物集合中）。
        ...
        for i in range( int((self.ld_d_range-self.car_radius)/self.ld_acc) + 2 ): # 多扫2次，让最大值超过self.ld_d_range，便于clamp
            # 更新雷达末端位置
        ...
        self.ld_scan_result = (self.ld_scan_result + self.car_radius).clamp(0,self.ld_d_range) #(N, ld_num)
        #注意： ld_d_range=100, # max scanning distance of lidar (cm)
    ...
    def _reward_function(self, current_a):
        ...
        self.tr_vec = (self.step_counter_vec > self.max_ep_steps)# truncated signal (N,)。   检查每个环境的步数计数器是否超过最大回合步数 max_ep_steps。
        self.exceed_vec = self.d2target_now > self.D # (N,)。 检查当前小车到目标点的距离 d2target_now 是否大于最大规划距离 D
        self.win_vec = self.d2target_now < self.target_area # (N,) 判断当前小车到目标点的距离是否小于目标区域半径 target_area
        self.collide_vec = (self.ld_result_grouped < self.collision_trsd).any(dim=-1)  # (N,)
        self.dw_vec = self.exceed_vec + self.win_vec + self.collide_vec # terminated signal (N,) 将超出最大距离、到达目标区域和碰撞这三种终止情况合并，形成终止信号。
        self.done_vec = self.tr_vec + self.dw_vec # (N,), used for AutoReset

        # current_a=[dx, dy, theta, v_linear, v_angular]
        ...
        #d2target_pre和d2target_now是前后两个时刻对target的相对位置
        ...
        #Note: 缩小和target距离和运动方向与target一致都有奖励
        if self.action_type == 'Discrete':
            ...
            #!!! 原代码有两处错误?! 这里的current_a[:,0] 并不是代表运动速度!
            R_forward = current_a[:,0].clip(0., 1.) #将数值强制限制在0到1之间
    ...
    def _stack_A_to_S(self, current_a, real_a, observation) -> torch.tensor:
        ...
        #将动作信息嵌入到状态中，从而构成一个“动作-状态”混合表示
        if self.action_type == 'Discrete': #这个可以去除? 改成只有if continuous就可以判断是否真可行?
    ...
    def _get_obs(self) -> torch.tensor:
        ...
        if self.Obs_refresh_counter > self.Obs_refresh_interval: #self.Obs_refresh_interval 默认设定值为1。
            self._obstacle_move()
            self.Obs_refresh_counter = 1
        ...
        # 3.制作observation
        observation_vec = torch.concat((self.car_state, self.ld_result_grouped), dim=-1) #(N, 5) cat (N, grouped_ld_num) = (N, 23)
    ...
    def step(self,current_a):
        ...
        '''Update car state: [dx, dy, theta, v_linear, v_angular]''' #current_a=[dx, dy, theta, v_linear, v_angular]
        # control delay mechanism
        self.ctrl_pipe.append(current_a)   # current_a is the action mapped by the current state。 当前状态下生成的动作 current_a（经过映射后的动作）被加入一个队列（控制管道）中。
        real_a = self.ctrl_pipe.popleft()  # real_a is the delayed action。 从队列中取出最旧的动作，这个动作由于排队等待，实际在此时才被执行。
        ...
        # #在后续更新机器人位置的过程中，x 坐标和 y 坐标的更新都需要使用同一个线速度（v_l），即用 v_l 分别乘以 dt·cos(θ) 和 dt·sin(θ)。因此，返回一个包含两个 v_l 可以方便地同时为 x 和 y 的更新提供同样的线速度值。
        ...
        self.d2target_now = (self.car_state[:, 0:2] - self.target_point).pow(2).sum(dim=-1).pow(0.5) # (N,), 执行动作后离目标点的欧氏距离,_reward_function和_Normalize会用
        ...
        # absolute coordinates will be transformed to relative distance to target
        # absolute orientation will be transformed to relative orientation
        relative_observation_vec = self._Normalize(observation_vec) # (N,22) 只有障碍物，没有小车的信息
        ...
        #dw_vec 到目的地/碰撞障碍物/碰撞边界
        #tr_vec 检查每个环境的步数计数器是否超过最大回合步数 max_ep_steps
    ...
    def _render_frame(self):
        ...
        # draw target area
        pygame.draw.circle(self.canvas, (100, 225, 100), self.target_point[0].cpu().numpy(), self.target_area, 2)
        ...
        for i in range(self.ld_num):
            ...
            pygame.draw.aaline(self.canvas, (255-e, 0, e), ld_real_sta_gd, ld_real_end_gd[i])
...
class Sparrow_PlayGround(Sparrow):
    ...
    def reset(self):
        ...
        #小车位置初始化
        self.d2target_pre = torch.zeros(self.N, device=self.dvc)  # Reset() 不产生奖励信号，这里pre随便赋值即可
        self.car_state.fill_(0)
        init_xy = self._car_loc_init(self.ri)

        # 重置global_path, waypoints, target_point
        self.global_path.clear() #这里会清空，鼠标点击加入的就不存在了
        ...
        # add noise to unormalized state：
        if self.noise:
            observation_vec += self.noise_magnitude*self._random_noise(1, (self.N,self.absolute_state_dim), self.dvc) # (N, absolute_state_dim)
        ...
        act_relative_observation_vec = self._stack_A_to_S(self.init_pre_action, self.init_pre_action, relative_observation_vec)
    ...
    def _reward_function(self, current_a):
        """只计算是否到达，是否碰撞"""
        self.win_vec = self.d2target_now < self.target_area # self.target_area是目标区域范围的半径
        self.collide_vec = (self.ld_result_grouped < self.collision_trsd).any(dim=-1)  # (N,)
        self.dw_vec = self.win_vec + self.collide_vec # terminated signal (N,)

    def step(self, current_a, relative_real_x_color, relative_real_y_color):
        ...
        # calculate and update the velocity of the car based on the delayed action and the Kinematic_model
        #感觉可以从这里入手，先做个简单版测试一下
        #if self.action_type == "Discrete": velocity = self._Discrete_Kinematic_model_vec(real_a) # [v_l, v_l, v_a], (N,3)
        #else: velocity = self._Continuous_Kinematic_model_vec(real_a) # [v_l, v_l, v_a], (N,3)

        print("relative_real_x_color: ", relative_real_x_color, "; relative_real_y_color: ", relative_real_y_color)
        ...
        #print("self.ctrl_interval: ", self.ctrl_interval)
        #print("In step(), velocity: ", velocity)
        
        #relative_real_x = [-500, +200]; relative_real_y = [-400, +400].
        relative_real_x_color_translation = relative_real_x_color+500
        relative_real_y_color_translation = relative_real_y_color+400

        # 从张量中提取标量值（假设 self.car_state 是 (N,3) 形状，N=1）
        #prev_x = self.car_state[0, 0].item()  # 使用 .item() 转为 Python 标量
        #prev_y = self.car_state[0, 1].item()
        #now_x = relative_real_x_color_translation
        #now_y = relative_real_y_color_translation
        
        # 将 now_x 和 now_y 组合成 (N,2) 的张量
        new_position = torch.tensor(
            [[relative_real_x_color_translation, relative_real_y_color_translation]],
            device=self.dvc,          
            dtype=torch.float32     )
        
        delta_y = new_position[:, 1] - self.car_state[:, 1]
        delta_x = new_position[:, 0] - self.car_state[:, 0]

        # 计算新朝向
        new_orientation = torch.atan2(delta_y, delta_x)  # 形状 (N,)

        '''self.car_state[:, 0:3] = torch.cat([
            new_position,                   # (N,2) 位置
            new_orientation.unsqueeze(1)    # (N,1) 朝向
        ], dim=1)'''

        self.car_state[:, 0:2] = torch.tensor(
            [[relative_real_x_color_translation, relative_real_y_color_translation]],  # 手动构造 (1, 2) 形状
            device=self.dvc,
            dtype=torch.float32
        )

        #self.car_state = torch.zeros((self.N, 5), device=self.dvc, dtype=torch.float32), N=1
        #print("self.car_state: ", self.car_state)
        ...
        # get next obervation
        observation_vec  = self._get_obs()    #包含小车和障碍物的信息

        # calculate reward, dw, tr, done signals
        self._reward_function(current_a)

        # change target point !重要!
        ...
        # absolute coordinates will be transformed to relative distance to target
        # absolute orientation will be transformed to relative orientation
        relative_observation_vec = self._Normalize(observation_vec) # (N,31) #包含小车和障碍物的信息

        # stack action_state to relative_observation_vec
        act_relative_observation_vec = self._stack_A_to_S(current_a, real_a, relative_observation_vec) # (N,31) -> (N,35)

        # render the current frame
        if self.render_mode == "human": self._render_frame()

        #! reset some of the envs based on the done_vec signal 理论上应该直接终止程序, 怎么终止？
        #self._AutoReset()
        #if self.collide_vec.any(): 用这个判断条件终止程序
    ...
    def _render_frame(self):
        ...
        if self.waypoints:
            # draw target point
            pygame.draw.circle(self.canvas,(100, 225, 100),self.waypoints[0],self.target_area,2)

            # draw waypoints
            for i in range(1, len(self.waypoints)):
                pygame.draw.circle(self.canvas,(255,200,15),self.waypoints[i],self.target_area,2)
        ...
        for event in ev:
            if event.type == pygame.MOUSEBUTTONDOWN:
                #替换成预设定的多个点测试一下
                xy = np.array(pygame.mouse.get_pos()) #通过鼠标点击获取下一个target_point的位置
                ...
                if distance > self.D:
                    ...
                    self.waypoints.append(xy) #存储着局部目标点。第一个点是car的初始位置。
