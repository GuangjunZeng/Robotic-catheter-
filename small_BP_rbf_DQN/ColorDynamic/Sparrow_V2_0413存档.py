...
default_cfg = dict(
    ...
    K=(0.9,0.9), # K_linear, K_angular   这两个系数用来调节机器人在控制命令执行时的惯性和响应特性。例如，在机器人接收到新的目标速度指令时，会将新的指令与当前状态（或先前的速度）按照 K 的权重进行混合，从而实现平滑过渡
...
class Sparrow_PlayGround(Sparrow):
    ...
    def step(self, current_a, relative_real_x_color, relative_real_y_color):
        ...
        try:
            relative_real_x_color, relative_real_y_color, t = message1['relative_real_x_color'], message1[
                'relative_real_y_color'], message1['t']
            #c1, c2 = test(relative_real_x_color, relative_real_y_color)
            c1, c2 = relative_real_x_color, relative_real_y_color
            t2 = t
            # c=a+b #调用函数
            data_to_send1 = {
                'c1': c1,
        ...
        #! 速度暂时没有同步成实际catheter end point速度
