dataSets = {'C:\Users\admin\Desktop\Labview2018\测试视频和图片\image4.xlsx'}; % 你自己的数据集
...
for dataSetID = 1:length(dataSets)
    % 读取excel数据
    data = xlsread(dataSets{dataSetID});
    ...
    if ctrl==2 && sqrt(sum((endPoints(1,:) - endPoints(2,:)) .^2)) > 2
        ...
        Paths{dataSetID} = Path;  % 保存每个数据集的Path
        ...
        title(['二值图像和其中心线 - Dataset: ', dataSets{dataSetID}]);
        xlabel('X');
        ylabel('Y');
