...
default_cfg = dict(
    ...
    Obs_R=10,  # maximal obstacle radius, cm
    Obs_V=7,  # maximal obstacle velocity, cm/s
...
class Sparrow():
    def __init__(self, **params):
        ...
        self.Dynamic_obs_canvas = pygame.Surface((2 * (self.Obs_R + self.w), 2 * (self.Obs_R + self.w)))  # 在 Pygame 中创建一个离屏画布（Surface），专门用来把动态障碍物“画”到这个画布上，后面再把画布上的像素转换成栅格点。在 Pygame 里，Surface 就是一块像素缓冲区，你可以把形状、颜色、图像都画到上面，再把它「贴」（blit）到屏幕或其它 Surface。
    ...
    def _static_obstacle_init(self):
        ...
        obs_np = pygame.surfarray.array3d(self.Static_obs_canvas)[:, :, 0] #得到的是每一个坐标位置的红色通道的强度
        '''


        # 手动定义固定障碍物（每个元组表示一个矩形的(x, y, width, height)）
        fixed_obstacles = [
            # 大型中央障碍物
            (300, 300, 150, 150),
        ...
        for obs in fixed_obstacles:
            pygame.draw.rect(self.Static_obs_canvas, (1, 1, 1), obs, 0)
    ...
    def _init_shape_parameters(self):
        """初始化形状固定参数（可扩展）"""
        self.shape_params = {
            'triangle': {'side_length': 20},  # 等边三角形边长
            'circle': {'radius': 15},  # 圆形半径
            'square': {'side': 20}  # 正方形边长
        }
    ...
    def _dynamic_obstacle_init(self):
        ...
        self.Obs_V_tensor = (self._random_noise(self.Obs_V, (self.N, self.O, 1, 2), self.dvc) * self.Obs_refresh_interval *\
                      self.ctrl_interval.reshape(self.N,1,1,1)).to(self.dvc).round().long() # 障碍物的速度, (N,O,1,2), 障碍物的速度大小和方向都是随机化的
        ...
        # 手动定义固定形状参数 (每个障碍物对应一个形状)
        fixed_shapes = [
            # 索引0: 圆形（圆心偏移，半径）
            {'type': 'circle', 'params': ((15, 15), 12)},
            # 索引1: 三角形（三个顶点坐标）
            {'type': 'polygon', 'params': [(0, 0), (30, 15), (0, 30)]},
            # 索引2: 正方形（中心坐标，边长）
            {'type': 'square', 'params': (15, 15, 25)},
            # 索引3: 菱形（四个顶点坐标）
            {'type': 'polygon', 'params': [(15, 0), (30, 15), (15, 30), (0, 15)]},
            # 索引4: 长方形（左上坐标，宽高）
            {'type': 'rect', 'params': (5, 5, 20, 25)},
            # 索引5: 正三角形（顶点坐标）
            {'type': 'polygon', 'params': [(15, 0), (30, 30), (0, 30)]}
        ]

        # 生成障碍物点云（保持原处理流程）
        uneven_obs_P_list = []
        P_np = np.zeros(self.O, dtype=np.int64)
        # 定义障碍物的全局位置（绝对坐标）
        global_positions = [
            (100, 150),  # 障碍物0的位置
            (500, 80),  # 障碍物1的位置
        ...
        for obs_idx in range(self.O):
            self.Dynamic_obs_canvas.fill((0, 0, 0))
            shape_def = fixed_shapes[obs_idx % len(fixed_shapes)]
            global_pos = global_positions[obs_idx % len(global_positions)]  # 获取全局位置

            # 根据类型绘制障碍物（使用局部坐标）
            if shape_def['type'] == 'circle':
                center, radius = shape_def['params']
                pygame.draw.circle(self.Dynamic_obs_canvas, (1, 1, 1), center, radius)
            elif shape_def['type'] == 'polygon':
                points = shape_def['params']
                pygame.draw.polygon(self.Dynamic_obs_canvas, (1, 1, 1), points)
            elif shape_def['type'] == 'square':
                x, y, size = shape_def['params']
                pygame.draw.rect(self.Dynamic_obs_canvas, (1, 1, 1), (x - size // 2, y - size // 2, size, size))
            elif shape_def['type'] == 'rect':
                x, y, w, h = shape_def['params']
                pygame.draw.rect(self.Dynamic_obs_canvas, (1, 1, 1), (x, y, w, h))

            # 腐蚀处理提取边界（修复可能为空的问题）
            obs_np = pygame.surfarray.array3d(self.Dynamic_obs_canvas)[:, :, 0]
            b_obs_np = ndimage.binary_erosion(obs_np, self.b_kernel).astype(obs_np.dtype)
            obs_np -= b_obs_np
            ...
            if np.sum(obs_np) == 0:
                # 如果没有边界点，使用原始形状
                obs_np = pygame.surfarray.array3d(self.Dynamic_obs_canvas)[:, :, 0]
                b_obs_np = np.zeros_like(obs_np)
                obs_np = obs_np.copy()  # 确保可写
                obs_np -= b_obs_np

            # 固定翻转模式（根据索引奇偶决定）
            if obs_idx % 2 == 0:
                obs_np = np.flip(obs_np, (0,))

            x_, y_ = np.where(obs_np == 1)

            # 应用全局位置偏移
            x_ = x_ + global_pos[0]
            y_ = y_ + global_pos[1]

            bound_gd = torch.tensor(np.stack((x_, y_), axis=1))
            uneven_obs_P_list.append(bound_gd)
            P_np[obs_idx] = bound_gd.shape[0]
    ...
    def _car_loc_init(self, N:int):
        ...
        while True:
            ...
            fixed_x = 600  # 示例固定 x 坐标
            fixed_y = 60 # 示例固定 y 坐标
...
class Sparrow_PlayGround(Sparrow):
    ...
    def step(self,current_a):
        ...
        print("A new loop: ...........", "self.step_counter_vec: ", self.step_counter_vec)
        print("previously, self.car_state[:, 0:3]: ", self.car_state[:, 0:3])

        orientation = self.car_state[:, 2]
        orientation = orientation.cpu().numpy()[0]
        ...
        print("next, self.car_state[:, 0:3]: ", self.car_state[:, 0:3])
        # keep the orientation between [0,2pi]
        self.car_state[:,2] %= (2 * torch.pi)
