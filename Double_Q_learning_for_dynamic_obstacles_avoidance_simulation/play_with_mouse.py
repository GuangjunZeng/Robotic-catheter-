...
parser.add_argument('--O', type=int, default=16, help='number of obstacles in each environment')
...
def main():
    ...
    while True:
        ...
        if envs.win_vec:
            a = 7*torch.ones((opt.N,), dtype=torch.long, device=opt.dvc) #在a_space中最后一个动作选择是停止。 在离散动作空间中，a是a_space的索引，不是action本身。
        else:
        ...
        # 读取action执行后的真实的relative end point
        # 对s中的信息进行修改
