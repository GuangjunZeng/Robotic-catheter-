...
class Sparrow():
    ...
    def _dynamic_obstacle_init(self):
        ...
        fixed_shapes = [
            # 索引0: 圆形（圆心偏移，半径）
            {'type': 'circle', 'params': ((80, 15), 12)},
            # 索引1: 三角形（三个顶点坐标）
            {'type': 'polygon', 'params': [(500, 550), (130, 65), (100, 80)]},
            # 索引2: 正方形（中心坐标，边长）
            {'type': 'square', 'params': (600, 50, 25)},
            # 索引3: 菱形（四个顶点坐标）
            {'type': 'polygon', 'params': [(50, 150), (65, 165), (50, 180), (35, 165)]},
            # 索引4: 长方形（左上坐标，宽高）
            {'type': 'rect', 'params':  (450, 450, 20, 25)},
            # 索引5: 正三角形（顶点坐标）
            {'type': 'polygon', 'params': [(450, 350), (265, 180), (235, 180)]}
        ]
        ...
        for obs_idx in range(self.O):
            ...
            shape_def = fixed_shapes[obs_idx % len(fixed_shapes)]  # 循环使用定义好的形状
            ...
            # 固定翻转模式（根据索引奇偶决定） #根据障碍物的索引 obs_idx 的奇偶性，决定是否对图像进行上下翻转。
            #if obs_idx % 2 == 0:
    ...
    def _car_loc_init(self, N:int):
        ...
        while True:
            ...
            fixed_x = 200  # 示例固定 x 坐标
            fixed_y = 330 # 示例固定 y 坐标
